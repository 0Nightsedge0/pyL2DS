...
''' TCP pattern check'''
def tcp_scan_checker2db2q(srcip, dstip, srcmac, dstmac, type, message, q2, lock, datetime, printdatetime, num):
    lock.acquire()
    q2.put([srcip, dstip, srcmac, dstmac, type, message])
    lock.release()
...
def tcp_scan_knew(ip, str1, tcp_scan_method_alerted):
    #print 'c', tcp_scan_method_alerted
...
def check_array_scan(tcp_array, check):
    tcp_array_len = len(tcp_array)
    check_len = len(check)

    #print 'tcp_array_len : ', tcp_array_len
    #print tcp_array
    ...
    i = 0
    j = 0
    while i < tcp_array_len:
        #print i, j
        if tcp_array[i] == check[j]:
            j += 1
            if j > (check_len-1):
                return True
        i += 1
...
def tcp_scan_checker(q2, lock, datetime, printdatetime, fnum,
    ...
    #print 'b',tcp_scan_method_alerted

    illegal_Connection_1 = ['SYN', 'RST']
    illegal_Connection_2 = ['SYN', 'FIN']
    illegal_Connection_3 = ['SYN', 'RA']

    illegal_Connection_array = [illegal_Connection_1, illegal_Connection_2, illegal_Connection_3]

    check_syn_scan_1 = ['SYN', 'SA', 'RST']
    check_syn_scan_2 = ['SYN', 'SA', 'FIN']
    check_syn_scan_3 = ['SYN', 'SA', 'RA']

    check_syn_scan_array = [check_syn_scan_1, check_syn_scan_2, check_syn_scan_3]

    check_connect_scan_1 = ['SYN', 'SA', 'ACK', 'RST']
    check_connect_scan_2 = ['SYN', 'SA', 'ACK', 'FIN']
    check_connect_scan_3 = ['SYN', 'SA', 'ACK', 'RA']

    check_connect_scan_array = [check_connect_scan_1, check_connect_scan_2, check_connect_scan_3]

    check_version_scan_1 = ['SYN', 'SA', 'RST', 'SYN', 'SA', 'ACK']
    check_version_scan_2 = ['SYN', 'SA', 'FIN', 'SYN', 'SA', 'ACK']
    check_version_scan_3 = ['SYN', 'SA', 'RA', 'SYN', 'SA', 'ACK']

    check_version_scan_array = [check_version_scan_1, check_version_scan_2, check_version_scan_3]

    #print 'tcp stack', tcp_stack

    for one_stack in tcp_stack:
        signal_detect = False
        detect_scan_method = None

        for version_scan in check_version_scan_array:
            if check_array_scan(one_stack[5:], version_scan):
                detect_scan_method = "Version Scan'"
                signal_detect = True

        for connect_scan in check_connect_scan_array:
            if signal_detect == True:
                break
            if check_array_scan(one_stack[5:], connect_scan):
                detect_scan_method = "Connect Scan"
                signal_detect = True

        for syn_scan in check_syn_scan_array:
            if signal_detect == True:
                break
            if check_array_scan(one_stack[5:], syn_scan):
                detect_scan_method = "SYN Scan"
                signal_detect = True

        for illegal_Connection in illegal_Connection_array:
            if signal_detect == True:
                break
            if check_array_scan(one_stack[5:], illegal_Connection):
                detect_scan_method = "illegal_Connection"
                signal_detect = True

        if signal_detect:
            tcp_scan_checker2db2q(one_stack[0], one_stack[1], one_stack[2], one_stack[3], detect_scan_method,
                                  'port: %s' % one_stack[4], q2, lock, datetime, printdatetime, fnum)
            tcp_scan_method_alerted.append([one_stack[0], detect_scan_method+' Scan'])
            fnum += 1
            #RS_connector.remote_shell(4, rsh[1], '')
                ...
                if i+2 > len(rsh)-1:
                    ...
                    if len(rsh) > 10:
                        if rsh[i+3] == 'SYN' and rsh[i+4] == 'SA' and rsh[i+5] == 'ACK':
                            tcp_scan_checker2db2q(rsh[0], rsh[1], rsh[2], rsh[3], 'Version SCAN',
                                                  'port: %s' % rsh[4], q2, lock, datetime, printdatetime, fnum)
    ...
    if pkt[TCP].flags == 2: # 2 = SYN
        ...
        for rsh in tcp_stack:
            if pkt[IP].src == rsh[0] and pkt[IP].dst == rsh[1] and pkt[TCP].dport == rsh[4]:
                ...
                break
        if checkpoint:
            tcp_stack.append([pkt[IP].src, pkt[IP].dst, l2_src_mac, l2_dst_mac, pkt[TCP].dport, 'SYN'])

    if pkt[TCP].flags == 18: #18 = SYN(2) + ACK(16)
        for rsh in tcp_stack:
            if pkt[IP].src == rsh[1] and pkt[IP].dst == rsh[0] and pkt[TCP].sport == rsh[4]:
                #print 'hi'
                rsh.append('SA')
                break
    ...
    if pkt[TCP].flags == 16: # ACK(16)
        if pkt[TCP].seq == 0:
            if tcp_scan_knew(pkt[IP].src, 'ACK', tcp_scan_method_alerted):
                ...
                tcp_scan_checker2db2q(srcip, dstip, l2_src_mac, l2_dst_mac, 'ACK SCAN',
    ...
    if pkt[TCP].flags == 1: # FIN(1)
        if pkt[TCP].ack == 0:
            if tcp_scan_knew(pkt[IP].src, 'FIN', tcp_scan_method_alerted):
                ...
                tcp_scan_checker2db2q(srcip, dstip, l2_src_mac, l2_dst_mac, 'FIN SCAN',
                                      'port: %s' % pkt[TCP].dport, q2, lock, datetime, printdatetime, num)
    ...
    if pkt[TCP].flags == 17: # FIN + ACK
        if pkt[TCP].seq == 0:
            if tcp_scan_knew(pkt[IP].src, 'Mainmon', tcp_scan_method_alerted):
                ...
                tcp_scan_checker2db2q(srcip, dstip, l2_src_mac, l2_dst_mac, 'Mainmon SCAN',
                                      'port: %s' % pkt[TCP].dport, q2, lock, datetime, printdatetime, num)
    ...
    if pkt[TCP].flags == 41: # FIN+PSH+URG
        if pkt[TCP].ack == 0:
            if tcp_scan_knew(pkt[IP].src, 'XMas', tcp_scan_method_alerted):
                tcp_scan_checker2db2q(srcip, dstip, l2_src_mac, l2_dst_mac, 'XMas Tree SCAN',
                                      'port: %s' % pkt[TCP].dport, q2, lock, datetime, printdatetime, num)
